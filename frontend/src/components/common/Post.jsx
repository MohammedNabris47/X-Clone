...
const Post = ({ post }) => {
  ...
  const isMyPost =
    authUser?._id && post?.user?._id && authUser._id === post.user._id;
  const isLiked =
  ...
  const { mutate: commentPost, isPending: isCommenting } = useMutation({
    mutationFn: async () => {
      try {
        ...
        // // Graceful handling for non-JSON responses (HTML error pages)
        ...
        //   }
        // } else {
        //   // Unexpected HTML or plain text can come from server fallback, e.g. 404 HTML.
        ...
        <div className="flex flex-col flex-1">
          ...
          <div className="flex justify-between mt-3">
            <div className="flex gap-4 items-center w-2/3 justify-between">
              ...
              >
                <FaRegComment className="w-4 h-4  text-slate-500 group-hover:text-white" />
              ...
              >
                <div className="modal-box rounded  bg-black">
                  ...
                  >
                    ...
                    />
                    <button className="btn bg-transparent hover:text-black duration-300 hover:bg-white rounded-full btn-sm text-white px-4">
                      {isCommenting ? (
              ...
              <div className="flex gap-1 items-center group cursor-pointer">
                <BiRepost className="w-6 h-6  text-slate-500 group-hover:text-white" />
                <span className="text-sm text-slate-500 group-hover:text-white">
                  0
                </span>
              </div>
              ...
              >
                ...
                {!isLiked && !isLiking && (
                  <FaRegHeart className="w-4 h-4 cursor-pointer text-slate-500 group-hover:text-white" />
                )}
                {isLiked && !isLiking && (
                  <FaRegHeart className="w-4 h-4 cursor-pointer text-white " />
                )}

                <span
                  className={`text-sm  group-hover:text-white ${
                    isLiked ? "text-white" : "text-slate-500"
                  }`}
                >
                  {post.likes.length}
                </span>
